...
def main(inp):
  ...
  idle_counters: List[int] = [0] * 50
  nat_packet: Tuple[int, int] = (0, 0)
  ...
  while True:
    for c in computers:
      next(c)
    if all(len(b) == 0 for b in input_buffers) and all(idle > 5 for idle in idle_counters):
      input_buffers[0].extend(nat_packet)
      _, y = nat_packet
      if y == last_y:
        return y
      last_y = y
